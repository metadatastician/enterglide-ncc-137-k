...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "enterglide-ncc-137-k"`
...
# an instantiation. The template submitted the literal `enterglide-ncc-137-k` as its own
...
# is set to `owner/enterglide-ncc-137-k` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `enterglide-ncc-137-k-2`,
